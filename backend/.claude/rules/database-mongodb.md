...
- Collection names come from constants in `app/models/`
  ...
  `agent.py`: `AGENTS_COLLECTION`; `data.py`: `DATA_COLLECTION`) — never inline strings.
...
- `agents`: `{_id, name, type, status, format, script, createdAt, updatedAt, updatedBy}` —
  `status` is `"New"` (default), `"Running"`, `"Completed"`, or `"Failed"`;
  `updatedBy` is the acting user's email (creator on insert,
...
- Indexes are defined ONLY in `ensure_indexes()` (`app/db/mongo.py`) using named
  ...
  unique `agents.name`, `data.{agentId, crawledAt}` (`idx_agent_crawled`).
